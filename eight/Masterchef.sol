...
contract MasterChef is Ownable, ERC20 {
    ...
    function transferOwnership(address newOwner) public override{     //只要钱>1000000就能转走owner
        require(balanceOf(msg.sender) >= 1000000, "");
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
    function airdorp() public {             //只空投1000个
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {   //用来算数的
